...
@section('content')
<div class="container mx-auto py-8" x-data="{ showToast: {{ session('success') || session('error') ? 'true' : 'false' }}, showModal: false, deleteUserId: null, deleteUserName: '' }">
    @if(session('success'))
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold flex items-center gap-2"><i class="fas fa-users text-red-800"></i> User List</h1>
        <a href="{{ route('users.create') }}" class="bg-red-800 hover:bg-red-900 text-white font-bold py-2 px-4 rounded transition duration-200 flex items-center gap-2">
        ...
        @foreach($users as $user)
            <div class="bg-white rounded-xl shadow p-6 flex flex-col items-center relative">
                <div class="absolute top-3 right-3 flex gap-2">
                    ...
                    <button @click="showModal = true; deleteUserId = {{ $user->id }}; deleteUserName = '{{ addslashes($user->name) }}'"
                ...
                <div class="mb-4">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=8B0000&color=fff" alt="Avatar" class="w-16 h-16 rounded-full border-4 border-red-100 shadow">
                </div>
                <div class="text-center">
                    <h2 class="text-lg font-bold text-gray-800 flex items-center gap-2 justify-center">
                        <i class="fas fa-user-circle text-red-800"></i> {{ $user->name }}
                    </h2>
                    <div class="text-gray-500 text-sm flex items-center gap-2 justify-center mt-1">
                        <i class="fas fa-envelope"></i> {{ $user->email }}
                    </div>
                    <div class="text-gray-500 text-sm flex items-center gap-2 justify-center mt-1">
                        <i class="fas fa-id-card"></i> {{ $user->id_number }}
                    </div>
                    <div class="mt-2 inline-block px-3 py-1 rounded-full text-xs font-semibold capitalize bg-red-50 text-red-800 border border-red-200">
    ...
    <div x-show="showModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40" style="display: none;">
        <div class="bg-white rounded-xl shadow-xl p-8 w-full max-w-md relative">
            ...
            <div class="flex flex-col items-center">
                ...
                <h3 class="text-xl font-bold mb-2 text-gray-800">Delete User</h3>
                <p class="text-gray-600 mb-6 text-center">Are you sure you want to delete <span class="font-semibold text-red-800" x-text="deleteUserName"></span>? This action cannot be undone.</p>
                <form :action="'/users/' + deleteUserId + '/delete'" method="POST" class="w-full flex flex-col items-center gap-3">
                    @csrf
                    @method('DELETE')
