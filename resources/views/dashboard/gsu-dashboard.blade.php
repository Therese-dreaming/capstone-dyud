...
@section('content')
<div class="min-h-screen w-full max-w-full bg-gradient-to-br from-gray-50 via-white to-red-50 overflow-x-hidden overscroll-x-none" x-data="gsuDashboardData()">
    <!-- GSU Welcome Banner -->
    <div x-show="showWelcome" x-transition class="bg-gradient-to-r from-red-600 to-red-800 text-white p-4 md:p-6 mb-6 rounded-xl shadow-lg relative overflow-hidden mx-4 mt-4">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 pb-8 overflow-x-hidden">
        <!-- Dashboard Header -->
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-3">
            <div>
                <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 flex items-center gap-3">
                    <div class="bg-gradient-to-r from-red-600 to-red-800 text-white p-3 rounded-xl shadow-lg">
                        <i class="fas fa-crown text-xl"></i>
                    </div>
                    GSU Dashboard
                </h1>
                <p class="text-gray-600 mt-2 text-xs sm:text-sm md:text-base">Manage assets, deployments, and system operations</p>
            </div>
            <div class="mt-3 sm:mt-0 flex items-center space-x-3 w-full sm:w-auto">
                @if($unreadNotifications > 0)
                <div class="bg-red-50 border border-red-200 text-red-800 px-3 sm:px-4 py-2 rounded-lg flex items-center gap-2">
                ...
                <button @click="printDashboard()" class="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg shadow-sm px-3 sm:px-4 py-2 border border-blue-700 hover:from-blue-700 hover:to-blue-800 transition-all duration-200 flex items-center gap-2" title="Print Dashboard">
        ...
        <div class="space-y-8">
            ...
            <div x-show="activeTab === 'assets'" x-transition>
                <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden w-full">
                    <div class="px-4 sm:px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-center sm:text-left w-full">
                        <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    <div class="p-4 sm:p-6 w-full">
                        ...
                        <div class="space-y-4 w-full">
                            ...
                            <a href="{{ route('gsu.assets.show', $asset) }}" class="w-full select-none flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-lg hover:bg-gray-100 active:bg-gray-200 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-0" style="touch-action: manipulation;" aria-label="View {{ $asset->asset_name ?? $asset->name }}">
                                <div class="flex items-center space-x-3 sm:space-x-4 min-w-0 w-full sm:w-auto">
                                    <div class="bg-blue-100 p-3 rounded-full">
                                        <i class="fas fa-cube text-blue-600"></i>
                                    </div>
                                    <div class="min-w-0">
                                        <h4 class="font-semibold text-gray-900 truncate break-words max-w-[11rem] sm:max-w-none">{{ $asset->asset_name ?? $asset->name }}</h4>
                                        <p class="text-xs sm:text-sm text-gray-600 font-mono break-all">{{ $asset->asset_code }}</p>
                                        <p class="text-[11px] sm:text-xs text-gray-500 truncate max-w-[12rem] sm:max-w-none">
                                            {{ $asset->location->building ?? 'No Location' }} - 
                                            {{ $asset->category->name ?? 'No Category' }}
                                        </p>
                                    </div>
                                </div>
                                <div class="w-full sm:w-auto sm:text-right text-center mt-1 sm:mt-0 flex items-center justify-between sm:block flex-shrink-0">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium sm:mb-1
                                    ...
                                    <p class="text-[11px] sm:text-xs text-gray-500 sm:mt-1">
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="border border-gray-200 rounded-lg p-3 sm:p-4 hover:shadow-md transition-shadow">
                                ...
                                <div class="flex items-center justify-between text-[11px] sm:text-xs text-gray-500">
                                    <span>
...
/* Print Styles */
@media print {
    button, nav, .no-print, .bg-gradient-to-r.from-red-600 {
        ...
        printDashboard() {
            ...
            printTitle.innerHTML = `
                <div style="text-align: center; margin-bottom: 30px; padding: 20px; border-bottom: 3px solid #DC2626;">
                    ...
                    <p style="color: #6B7280; font-size: 14px;">
                        Generated by: {{ Auth::user()->name ?? 'GSU User' }}
                    </p>
                </div>
            `;
